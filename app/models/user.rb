class User < ActiveRecord::Base
  ...
  def self.find_for_facebook_oauth(auth, signed_in_resrouce=nil)
    ...
    unless user
      user = User.create(
        first_name: auth.extra_info.raw_info.first_name,
        last_name: auth.extra_info.raw_info.last_name,
  ...
  def self.new_with_session(params, session)
    super.tap do |user|
      if data = session["devise.facebook_data"] && session["sevise.facebook_data"]["extra"]["raw_info"]
